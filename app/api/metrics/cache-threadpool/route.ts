...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await metricsService.getCacheThreadPool(timeRange, step);
    return NextResponse.json(data);
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Failed to fetch cache/threadpool metrics';
