...
| 변수명 | 기본값 | 노출 범위 | 설명 |
| :--- | :--- | :--- | :--- |
| `CLUSTER_API_URL` | `http://localhost:18080` | 서버 전용 | Next.js API Routes 프록시가 백엔드를 호출하는 기본 URL. **브라우저에 노출되지 않습니다.** |
| `NEXT_PUBLIC_APP_NAME` | `Elasticsearch Management Tool` | 클라이언트 | 앱 표시명 |
| `NEXT_PUBLIC_APP_VERSION` | `1.0.0` | 클라이언트 | 앱 버전 |
| `NEXT_PUBLIC_APP_COOKIE_SECURE` | `false` | 클라이언트 | 운영 HTTPS 환경에서 `true`로 설정. Secure 쿠키 플래그 제어. |
| `NEXT_PUBLIC_APP_PROFILE` | `dev` | 클라이언트 | **배포 환경 구분 배지.** 헤더 및 로그인 화면에 시각적 환경 표시. `dev` (개발), `stg` (검증), `prod` (운영 — ⚠️ 경고 아이콘 강조 표시). |
| `NEXT_PUBLIC_AUTH_TOKEN_EXPIRE_HOURS` | `24` | 클라이언트 | **JWT 세션 쿠키 만료 시간 (단위: 시간).** 백엔드 API의 `JWT_ACCESS_TOKEN_EXPIRE_HOURS`와 **반드시 동일하게 설정**해야 합니다. |

> [!CAUTION]
> `NEXT_PUBLIC_AUTH_TOKEN_EXPIRE_HOURS` (UI의 쿠키 만료)와 `JWT_ACCESS_TOKEN_EXPIRE_HOURS` (API의 JWT 유효시간)는 하나의 세션을 이루는 쌍입니다. 두 값이 다를 경우 쿠키가 살아있는데 JWT 토큰은 만료되었거나, 반대로 쿠키는 사라지는 문제가 발생할 수 있습니다.

---

### 3. 브라우저 노출 경계 (`NEXT_PUBLIC_`) 규칙
...
### 4. `.env` 파일 대신 기동(실행) 시점에 동적 주입하는 방법
...
서버 구동 후 웹 브라우저에서 [http://localhost:3000](http://localhost:3000)으로 접속하여 사용하실 수 있습니다.

> [!NOTE]
> **초기 로그인 계정:**
> API 서버 최초 기동 시 등록된 사용자가 없으면 `admin` / `admin` 계정이 자동으로 생성됩니다.
> 보안을 위해 환경 구성 후 반드시 비밀번호를 변경하십시오.
