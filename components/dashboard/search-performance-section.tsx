  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <AreaChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
        ...
        {nodeNames.map((name, idx) => (
          <Area key={name} type="monotone" dataKey={name} stroke={NODE_COLORS[idx % NODE_COLORS.length].stroke} fill={`url(#search-grad-${idx})`} strokeWidth={1.5} dot={false} isAnimationActive={false} />
        ))}
      </AreaChart>
    </ResponsiveContainer>
  );
