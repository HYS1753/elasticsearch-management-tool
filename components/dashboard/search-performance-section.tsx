...
import type { SearchPerformanceResponse, MetricSeries } from '@/types/metrics';
      ...
      <AreaChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
        <defs>
          {nodeNames.map((name, idx) => (
            <linearGradient key={name} id={`search-grad-${idx}`} x1="0" y1="0" x2="0" y2="1">
        ...
        {nodeNames.map((name, idx) => (
          <Area key={name} type="monotone" dataKey={name} stroke={NODE_COLORS[idx % NODE_COLORS.length].stroke} fill={`url(#search-grad-${idx})`} strokeWidth={1.5} dot={false} animationDuration={800} animationEasing="ease-out" />
        ))}
      </AreaChart>
    </ResponsiveContainer>
  );
...
interface SearchPerformanceSectionProps {
  data: SearchPerformanceResponse | null;
  loading: boolean;
}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <MetricChartCard
          ...
          loading={loading}
        >
          {data && renderAreaChart(data.query_rate, (v) => `${v.toFixed(1)}/s`)}
        </MetricChartCard>
        <MetricChartCard
          ...
          loading={loading}
        >
          {data && renderAreaChart(data.query_latency, (v) => `${(v * 1000).toFixed(0)}ms`)}
        </MetricChartCard>
        <MetricChartCard
          title="Fetch Rate"
          ...
          loading={loading}
        >
          {data && renderAreaChart(data.fetch_rate, (v) => `${v.toFixed(1)}/s`)}
        </MetricChartCard>
        <MetricChartCard
          title="Fetch Latency"
          ...
          loading={loading}
        >
          {data && renderAreaChart(data.fetch_latency, (v) => `${(v * 1000).toFixed(0)}ms`)}
        </MetricChartCard>
      </div>
    </div>
  );
