      ...
      <CardContent className="pt-6">
        ...
        <div className="overflow-x-auto">
          <div className="min-w-max">
            {/* Header Row */}
            <div className="flex border-b-2 border-slate-300 dark:border-slate-800">
              <div className="w-[256px] px-4 py-2 font-semibold text-slate-700 dark:text-slate-300 border-r-2 border-slate-300 bg-slate-50 dark:bg-slate-900 flex items-center gap-2 min-h-[80px] flex-shrink-0 sticky left-0 z-10">
                <Server className="h-4 w-4 text-slate-600 dark:text-slate-400" />
                Nodes
              </div>
              
              <div className="flex bg-gradient-to-r from-slate-50 to-slate-100/50 dark:from-slate-900 dark:to-slate-900/50">
                {indices.map((index) => {
                  const totalShards = Object.values(index.shards_by_node).flat().length;
                  return (
                    <div key={index.index} className="w-[220px] px-4 py-2 text-left border-r border-slate-200/60 dark:border-slate-800 flex-shrink-0">
                      <div className="font-semibold text-slate-800 dark:text-slate-200 text-sm truncate" title={index.index}>
                        {index.index}
                      </div>
            ...
            {has_unassigned_shards && (
              <div className="flex border-b border-slate-200/60 dark:border-slate-800 bg-amber-50/40 hover:bg-amber-50/60 dark:bg-amber-500/5 dark:hover:bg-amber-500/10 transition-colors">
                <div className="w-[256px] px-4 py-4 border-r-2 border-slate-300 dark:border-slate-700 flex-shrink-0 sticky left-0 z-10 bg-amber-50 dark:bg-amber-900/10">
                ...
                <div className="flex">
                  {indices.map((index) => (
                    <div key={`unassigned-${index.index}`} className="w-[220px] px-4 py-4 flex items-start justify-start border-r border-slate-200/60 dark:border-slate-800 flex-shrink-0">
                      {index.unassigned.length > 0 ? (
            ...
            {/* Node Rows */}
            {nodes.map((node) => (
              <div key={node.id} className="flex border-b border-slate-200/60 hover:bg-slate-50/50 dark:bg-slate-900/50 transition-colors">
                <div 
                  className="w-[256px] px-4 py-4 border-r-2 border-slate-300 bg-white dark:bg-slate-950 cursor-pointer group flex-shrink-0 sticky left-0 z-10"
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-slate-800 dark:text-slate-200 text-sm truncate">{node.name}</span>
                        {node.is_master && (
                          <Badge variant="default" className="text-[10px] px-1.5 py-0">Master</Badge>
                        )}
                      </div>
                      <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 truncate">{node.host}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex">
                  {indices.map((index) => {
                    ...
                    return (
                      <div key={`${node.id}-${index.index}`} className="w-[220px] px-4 py-4 flex items-start justify-start border-r border-slate-200/60 dark:border-slate-800 flex-shrink-0">
                        {nodeShards.length > 0 ? (
                          <div className="flex flex-wrap gap-1.5 max-w-full">
